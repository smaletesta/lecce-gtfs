...
module.exports = (function(){
	...
	var gtfs;
        ...
        // check if trip is in service today
        var isTripInService = function(tripId) {
                var now = new Date();
		dateInt = parseInt(now.toFormat("YYYYMMDD"),10);
                serviceId = gtfs.getTripById(tripId).service_id;
                calendar = gtfs.getCalendarById(serviceId);
		switch (now.getDay())
		{
		case 0:
			serviceDay = calendar.sunday;
			break;
                case 1:
                        serviceDay = calendar.monday;
                        break;
                case 2:
                        serviceDay = calendar.tuesday;
                        break;
                case 3:
                        serviceDay = calendar.wednesday;
                        break;
                case 4:
                        serviceDay = calendar.thursday;
                        break;
                case 5:
                        serviceDay = calendar.friday;
                        break;
                case 6:
                        serviceDay = calendar.saturday;
                        break;
		}
		if ((parseInt(calendar.start_date,10) <= dateInt) && (dateInt <= parseInt(calendar.end_date,10)) && parseInt(serviceDay,10))
			return 1;
		else
			return 0;	
	}
	...
	var calculateTripProgess = function(time, delta) {
		...
		for (var i in trips) {
			if (isTripInService(trips[i][0].trip_id)) {
...
//console.log("tripid: "+trips[i][0].trip_id+" start: "+startTime+" end: "+endTime+ " duration "+trips[i].duration)
				...
				if (gtfs.getTripById(trips[i][0].trip_id) && gtfs.getTripById(trips[i][0].trip_id).direction_id==1) {
    ...
    var d = new Date();
		callback({
			trips : calculateTripProgess(d, 10000),
			timestamp :  d.getTime()
		});
	}	
